...
### Manual Patent Search Cross-check

The automated NCBI Protein PAT search is paired with manual patent-site
validation so the search process can be demonstrated and audited.

| Website | Manual role | Example query |
|---------|-------------|---------------|
| NCBI Protein | Reproduce the exact sequence retrieval search | `(anti-PD-1 OR anti-PDCD1 OR nivolumab OR pembrolizumab OR cemiplimab) AND (antibody OR immunoglobulin OR VH OR VHH OR VL OR scFv OR nanobody OR Fab OR heavy chain OR light chain OR CDR OR variable region) AND "PAT"[Division]` |
| USPTO Patent Public Search | Check official patent / application context | `"PD-1" antibody variable region` |
| USPTO Issued and Published Sequences | Retrieve official sequence listings when a patent/publication number is known | search by patent or publication number |
| Google Patents | Rapid family-level cross-check and `SEQ ID NO` search | `("PD-1" OR PDCD1 OR pembrolizumab OR nivolumab) (antibody OR immunoglobulin OR scFv OR Fab OR nanobody) country:US` |

See `docs/patent_manual_search_workflow.md` for the live demonstration steps.

### Search Audit and Patent Validation Queue

The manual patent-site validation layer is represented by three generated CSVs:

| File | Rows | Purpose |
|------|------|---------|
| `outputs/patent_search_audit.csv` | 64 | Target-level query audit, NCBI hit/kept counts, retmax cap status, and manual NCBI/Google Patents URLs |
| `outputs/patent_validation_queue.csv` | 2,679 | Row-level NCBI Patent validation queue with patent number, accession, inferred sequence label, validation questions, and blank manual fields |
| `outputs/patent_validation_patent_summary.csv` | 172 | Patent-level summary for batching USPTO / Google Patents review |
...
They do not claim that USPTO or Google Patents have already been parsed. They
make the next manual validation step explicit and reproducible.
